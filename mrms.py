...
with tempfile.TemporaryDirectory() as tmp:
    mrms_files = sorted(list(glob(f"mtarchive.geol.iastate.edu/{year}/*/*/mrms/ncep/PrecipRate/*.gz")))
    ...
    for data in tqdm(
        ...
        if len(datasets) >= 15:
            ...
            print(dataset)
            if first:
                dataset.to_zarr(f"{year}.zarr", consolidated = True, compute = True, mode="w", **extra_kwarfs)
                first = False
            else:
                dataset.to_zarr(f"{year}.zarr", consolidated = True, compute = True, mode="a", **zarr_mode_to_extra_kwargs["a"])
            datasets = []
